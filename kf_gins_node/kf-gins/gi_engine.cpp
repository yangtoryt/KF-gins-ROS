...
GIEngine::GIEngine(GINSOptions &options) {
    ...
    void GIEngine::initialize(const NavState &initstate, const NavState &initstate_std) {
        ...
        auto ensure_vec3 = [](Eigen::Vector3d &v, const Eigen::Vector3d &def) {
            for (int i = 0; i < 3; ++i) {
                if (!std::isfinite(v[i]) || v[i] < 0.0) v[i] = def[i];
            }
        };
        ...
        for (int i = 0; i < 3; ++i) {
            if (!std::isfinite(imu_std.gyrbias[i]) || imu_std.gyrbias[i] < 0.0) imu_std.gyrbias[i] = DEF_IMU_STD.gyrbias[i];
            if (!std::isfinite(imu_std.accbias[i]) || imu_std.accbias[i] < 0.0) imu_std.accbias[i] = DEF_IMU_STD.accbias[i];
            if (!std::isfinite(imu_std.gyrscale[i]) || imu_std.gyrscale[i] < 0.0) imu_std.gyrscale[i] = DEF_IMU_STD.gyrscale[i];
            if (!std::isfinite(imu_std.accscale[i]) || imu_std.accscale[i] < 0.0) imu_std.accscale[i] = DEF_IMU_STD.accscale[i];
        }
...
int GIEngine::isToUpdate(double imutime1, double imutime2, double updatetime) const {
    ...
    void GIEngine::EKFUpdate(Eigen::MatrixXd &dz, Eigen::MatrixXd &H, Eigen::MatrixXd &R) {
        ...
        if (S.size() == 0) invertible = false;
        else {
            // 尝试求逆，捕获异常
            // 使用 .inverse() 但在数值不良时可能产生 inf/nan
            // 我们先检查行列式/对角是否有效
            if (!S.allFinite()) invertible = false;
        }
